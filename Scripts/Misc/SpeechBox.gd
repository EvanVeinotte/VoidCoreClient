...
func plusOneLetter():
	...
	if(totaltext[positioninthetext] != " " and Constants.inworld):
	...
	if(positioninthetext >= len(totaltext)):
		doneadding = true
		if(autoclear):
			get_node("SayTimer").start(Constants.SPEECH_BOX_SPEED)
	else:
		get_node("LetterTimer").start(Constants.SPEECH_BOX_LETTER_SPEED)
