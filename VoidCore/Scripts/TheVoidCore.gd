	...
	if(spawningitem):
		...
		if(itemspawntimer >= 1):
			...
			if(postoplace):
				...
				myworld.instantiateNewObject(postoplace, thisobjecttypeid, thisobjectid, extradata, true, false)
				#myworld.instantiateNewObject(postoplace, thisobjecttypeid, thisobjectid, extradata, true, [voidcenterpos, Vector2(newxpos, newypos)])
			else:
